...
function ClickB(name, worth) {
    ...
    else { msg("Not Unlocked!", "This B is not unlocked. Press the unlock button above the B to unlock it."); }
}
...
function UnlockB(name, cost) {
    if (get(name) == 'false') {  
        ...
        msg("Already Unlocked!", "Click the B button to get points.");
        if (!isMobile.any()) {
            FB.ui({
              method: 'share',
              display: 'popup',
...
var data = {
    reset : {
        ...
        hard : function() {
            ...
            if (get("multiplier") !== undefined) set('multiplier', (get("multiplier") * 0.05)); 
						else set('multiplier', 1);
						data.reset.soft();
        }
    },
    load : function() {
        if (get("played328") === undefined) data.reset.hard();   
        else if (get("lastPlay") !== undefined) {
            set('points', add(get('points'), ((get('interest') * 0.05) * timeSince(get('lastPlay')))));
            msg('While you were away, you earned ' + ((get('interest') * 0.05) * timeSince(get('lastPlay'))) + ' points!');
        }
				if (get("lastPlayDay") !== undefined && get("lastPlayDay") != d.getDate()) {
					...
					msg("You earned " + get("daysInARow") + " times your points from your daily gift!");
				}
    },
    b64 : {
        import : function() {
        ...
        export : function() {
            ...
            get('awesome') + ';' + get('golden') + ';' + get('pattern') + ';' + get('gradient') + ';' + get('disco') + ';' + get('unlocked');
            prompt('This is your game save code. Import it on another computer.', format.base64(saveCode));
        }
    }
}

function ConfirmHR() { if (ask("Reset Everything?") === true) data.reset.hard(); }

function ConfirmSR() { if (ask("Reset Progress but not Achievements?") === true) data.reset.soft(); }
...
function ChangeUsername() {
    ...
    else msg("Username Changed!", "Your username has been changed to " + get('username') + ".");
}
...
var b = {
    ...
    refresh : function() {
        ...
        if (localStorage.getItem(b.list.list[get("bPosition")]) == 'false') b.vars.unlock().innerHTML = "Unlock<br />(" + format.giant(b.list.b[b.list.list[get("bPosition")]].cost) + ")";
				else {
    ...
    loop : {
        next : function() {
            if (get("bPosition") == 15) set("bPosition", 0);
            else set("bPosition", add(get("bPosition"), 1));
            b.refresh();
        },
        previous : function() {
            if (get("bPosition") == 0) set("bPosition", 15);
