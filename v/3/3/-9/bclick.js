...
var refresh = {
    numbers : function() {
            ...
            else { id('Points').innerHTML = 'Points: ' + m.giant(get('points')); document.title = m.giant(get('points')) + " Points - bClick";}
            ...
            window.requestAnimationFrame(function(){refresh.numbers});
    },
    ...
    all : function() {
        ...
        lastCalledTime = Date.now();
        fps = 1 / delta;
        b.refresh(); refresh.achievements(); refresh.events(); 
        setTimeout(window.requestAnimationFrame(refresh.all), 1000 / fps);
    }
}; 
