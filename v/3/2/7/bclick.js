...
var refresh = {
    numbers : function() {
            if (get('points') >= Math.pow(10, 307)) { set('points', format.eg(Math.pow(10, 308))); id('Points').innerHTML = "Points: Infinity"; document.title = "Infinity Points - bClick";} 
...
var data = {
    ...
    load : function() {
        if (get("played327") == undefined) {
            // set("played327Tut", "false");
            data.reset.hard();   
        }
        // if (get("played327Tut") == "false") {
        //     tutorial.start();
        // }
    },
    b64 : {
...
function SubmitScore() {  
     // if (ask("Win with the username " + get("username") + "?") == true) {
        var username = prompt('Enter username to submit');
        if (username != undefined && username != null && username != '') {
            web("https://playbclick.com/pages/win/win.php?username=" + username + "&points=" + toFixed(get('points')));
            data.reset.soft();
