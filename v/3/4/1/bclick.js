...
function weird() {
    ...
    for (i = 0; i < Math.ceil(Math.random() * 5); i++) {
        weirdness += Math.ceil(Math.random() * Math.random() * 10).toString().charAt(0);
    }
    var times = (5 - weirdness.length);
    for (i = 0; i < times; i++) {
...
var data = {
    reset : {
        ...
        update : function() { /*set('thirteen', false); set('star', false); set('impossible', false); set('played341-925216', true);*/ },
    ...
    load : function() {
		...
		if (get("lastPlayDay") !== undefined && get("lastPlayDay") != d.getDate()) {
			...
			else if (get("lastPlayDay") > d.getDate && get("lastPlayDay") != 30) { 
			    set("daysInARow", 1);
				log("CHEATER!!!!!!!");
        ...
        if (get("username") == 'null' || get("username") == undefined || get("username") == '' || get("username") == ' ') set('username', 'bClicker' + weird());
...
var b = {
    ...
    refresh : function() {
        ...
        for (i = 0; i < b.list.soon.list.length; i++) {
            if (b.list.soon.b[b.list.soon.list[i]].date <= new Date()) {
                location.reload();
            } else {
                id('comingSoonDate' + b.list.soon.list[i]).innerHTML = neatTime((b.list.soon.b[b.list.soon.list[i]].date - new Date()) / 1000);
            }
        }
    },
    loop : {
...
function create(name, worth, cost, color, date) {
    this.worth = worth;
    this.cost = cost;
    this.color = color;
    this.name = name;
    ...
    if (date != undefined && date > new Date()) {
        ...
        b.list.soon.b[name] = { name, worth, cost, color, date };
        id('comingSoonBList').innerHTML += '<div class="comingSoonB" id="comingSoon' + name + '">' + name + ' B</div><div class="comingSoonDate" id="comingSoonDate' + name + '">' + (date - new Date()) + '</div>';
        return;
    }
    b.list.list.push(name);
    set('bAmount', m.add(get('bAmount'), 1));
...
b.list.b.shark = new create('shark', 10000, 10000000, "#060");
b.list.b.fancy = new create('fancy', 25000, 50000000, "#6CF");
b.list.b.lightning = new create('lightning', 100000, 2500000000, "#06C");
...
b.list.b.trump = new create('trump', Math.pow(10, 54), Math.pow(10, 57), "#0000FF", new Date('Mon Sep 26 2016 20:00:00 GMT-0500 (Central Daylight Time)'));
b.list.b.ghost = new create('ghost', Math.pow(10, 57), Math.pow(10, 60), "#444444", new Date('Sat Oct 1 2016 12:00:00 GMT-0500 (Central Daylight Time)'));
b.list.b.election = new create('election', Math.pow(10, 60), Math.pow(10, 63), "#0000FF", new Date('Mon Oct 24 2016 12:00:00 GMT-0500 (Central Daylight Time)'));
